...
test("FindingsManager", async (t) => {
	...
	await t.test("populateFindings should extract various tags", async () => {
		...
		const tags = [
			...
			{ tagName: "drop", attrs: [{ name: "file", value: "old.js" }] },
			{
				tagName: "remark",
				isMock: true,
				childNodes: [{ value: "some remark" }],
			},
			{
				tagName: "edit",
				attrs: [{ name: "file", value: "edit.js" }],
				isMock: true,
				childNodes: [{ value: "patch" }],
			},
			{
				tagName: "delete",
				attrs: [{ name: "file", value: "del.js" }],
				isMock: true,
			},
			{
				tagName: "prompt_user",
				isMock: true,
				childNodes: [{ value: "Question?" }],
			},
	...
	await t.test(
		"resolveOutstandingFindings should handle all categories",
		async (t) => {
			const runId = "r1";
			...
			const infoTags = [
				...
				{
					tagName: "info",
					attrs: [{ name: "notification", value: "3" }],
					isMock: true,
					childNodes: [{ value: "yes" }],
				},
			];
	...
	await t.test("applyDiff should handle create and delete types", async (t) => {
		...
		const exists = await fs
			.access(join(projectPath, "hello.txt"))
			.then(() => true)
			.catch(() => false);
