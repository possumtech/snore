...
test("ResponseParser", async (t) => {
	...
	await t.test("mergePrefill should handle content starting with prefill", () => {
		assert.strictEqual(parser.mergePrefill("<tasks>", "<tasks>- [ ]"), "<tasks>- [ ]");
	});

	await t.test("mergePrefill should prepend prefill for partial completions", () => {
		assert.strictEqual(parser.mergePrefill("<tasks>\n- [", "] task"), "<tasks>\n- [] task");
		assert.strictEqual(parser.mergePrefill("<tasks>\n- [", "x] task"), "<tasks>\n- [x] task");
	});

	await t.test("mergePrefill should prepend prefill if <tasks> is missing", () => {
		assert.strictEqual(parser.mergePrefill("<tasks>", "no tasks"), "<tasks>no tasks");
	});

	await t.test("appendAssistantContent should create and append elements", () => {
		...
		parser.appendAssistantContent(turnObj, "content", "some <b>bold</b> text");
		const contentEl = assistant.getElementsByTagName("content")[0];
		assert.ok(contentEl);
		assert.strictEqual(contentEl.childNodes.length, 3); // "some ", <b>, and " text"
		assert.strictEqual(contentEl.getElementsByTagName("b")[0].textContent, "bold");
	});
	...
	await t.test("parsePromptUser should handle question with checklist options", () => {
		const node = { isMock: true, childNodes: [{ value: "Choose one:\n- [ ] Opt 1: Desc 1\n- [ ] Opt 2: Desc 2" }] };
		const result = parser.parsePromptUser(node);
	...
	await t.test("parseActionTags should extract various tags", () => {
		...
		assert.strictEqual(readTag.attrs.find(a => a.name === "file").value, "test.js");
	});
