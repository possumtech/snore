...
test("ClientConnection Expanded Coverage", async (t) => {
	...
	const mockDb = {
		...
		get_run_by_id: { get: async () => ({ id: "r1", session_id: "s1", config: "{}" }) },
		...
		get_repo_map_file: { get: async () => ({ is_buffered: 0, size: 0, visibility: "mappable" }) },
	...
	const createMockConn = () => {
		...
		const ws = {
			on: () => {},
			send: (d) => { state.sent = JSON.parse(d); },
			readyState: 1,
		};
	...
	await t.test("should handle basic lifecycle: ping, discover, init", async () => {
		...
		await conn.handleMessageForTest(Buffer.from(JSON.stringify({ jsonrpc: "2.0", method: "discover", id: 2 })));
		assert.ok(state.sent.result.methods.init);

		await conn.handleMessageForTest(Buffer.from(JSON.stringify({ 
			jsonrpc: "2.0", method: "init", id: 3, 
	...
	await t.test("should handle model and file metadata", async () => {
		...
		// Init first
		await conn.handleMessageForTest(Buffer.from(JSON.stringify({ 
			jsonrpc: "2.0", method: "init", id: 1, params: { projectPath: "/tmp" } 
		})));

		await conn.handleMessageForTest(Buffer.from(JSON.stringify({ jsonrpc: "2.0", method: "getModels", id: 2 })));
		...
		await conn.handleMessageForTest(Buffer.from(JSON.stringify({ 
			jsonrpc: "2.0", method: "fileStatus", id: 4, params: { path: "a.js" } 
		})));
	...
	await t.test("should handle run lifecycle: start, resolve, affirm, abort", async () => {
		const { conn, state } = createMockConn();
		await conn.handleMessageForTest(Buffer.from(JSON.stringify({ 
			jsonrpc: "2.0", method: "init", id: 1, params: { projectPath: "/tmp" } 
		})));
		...
		await conn.handleMessageForTest(Buffer.from(JSON.stringify({ 
			jsonrpc: "2.0", method: "startRun", id: 2, params: { type: "ask" } 
		})));
		assert.ok(state.sent.result);

		// Mock global fetch for LLM call inside resolve
		globalThis.fetch = async () => new Response(JSON.stringify({
			choices: [{ message: { role: "assistant", content: "<tasks>- [x] ok</tasks><summary>Done</summary>" } }],
			usage: { total_tokens: 10 }
		}));

		await conn.handleMessageForTest(Buffer.from(JSON.stringify({ 
			jsonrpc: "2.0", method: "run/resolve", id: 3, params: { runId: "r1", resolution: { action: "accepted" } } 
		})));
		// Success check
		assert.ok(!state.sent.error);

		await conn.handleMessageForTest(Buffer.from(JSON.stringify({ 
			jsonrpc: "2.0", method: "run/affirm", id: 4, params: { runId: "r1" } 
		})));
		assert.strictEqual(state.sent.result.status, "ok");

		await conn.handleMessageForTest(Buffer.from(JSON.stringify({ 
			jsonrpc: "2.0", method: "run/abort", id: 5, params: { runId: "r1" } 
		})));
		assert.strictEqual(state.sent.result.status, "ok");
	});

	await t.test("should handle session configuration: prompt, persona, skills", async () => {
		const { conn, state } = createMockConn();
		await conn.handleMessageForTest(Buffer.from(JSON.stringify({ 
			jsonrpc: "2.0", method: "init", id: 1, params: { projectPath: "/tmp" } 
		})));
		...
		await conn.handleMessageForTest(Buffer.from(JSON.stringify({ 
			jsonrpc: "2.0", method: "systemPrompt", id: 2, params: { text: "new prompt" } 
		})));
		assert.strictEqual(state.sent.result.status, "ok");

		await conn.handleMessageForTest(Buffer.from(JSON.stringify({ 
			jsonrpc: "2.0", method: "persona", id: 3, params: { text: "new persona" } 
		})));
		assert.strictEqual(state.sent.result.status, "ok");

		await conn.handleMessageForTest(Buffer.from(JSON.stringify({ 
			jsonrpc: "2.0", method: "skill/add", id: 4, params: { name: "s1" } 
		})));
		assert.strictEqual(state.sent.result.status, "ok");

		await conn.handleMessageForTest(Buffer.from(JSON.stringify({ 
			jsonrpc: "2.0", method: "skill/remove", id: 5, params: { name: "s1" } 
		})));
		assert.strictEqual(state.sent.result.status, "ok");
	});
	...
	await t.test("should handle ask/act/run with buffer sync", async () => {
		...
		await conn.handleMessageForTest(Buffer.from(JSON.stringify({ 
			jsonrpc: "2.0", method: "ask", id: 2, params: { prompt: "hi", projectBufferFiles: ["a.js"] } 
		})));
		assert.strictEqual(state.sent.id, 2);

		await conn.handleMessageForTest(Buffer.from(JSON.stringify({ 
			jsonrpc: "2.0", method: "act", id: 3, params: { prompt: "do it" } 
		})));
		assert.strictEqual(state.sent.id, 3);

		await conn.handleMessageForTest(Buffer.from(JSON.stringify({ 
			jsonrpc: "2.0", method: "run", id: 4, params: { prompt: "go" } 
		})));
	...
	await t.test("should throw error if project not initialized", async () => {
		const { conn, state } = createMockConn();
		await conn.handleMessageForTest(Buffer.from(JSON.stringify({ 
			jsonrpc: "2.0", method: "getFiles", id: 1 
		})));
		assert.ok(state.sent.error);
