...
describe(
	"E2E: yolo mode auto-resolves and self-executes (@yolo_mode)",
	{ concurrency: 1 },
	() => {
		if (!model) {
		...
		it(
			"yolo run reaches terminal and routes proposals server-side",
			{ timeout: TIMEOUT },
			async () => {
				...
				const startRes = await client.call("set", {
					path: "run://",
					body:
						"Update FACTS.md so it lists the developer details from this project's data file. Then complete.",
