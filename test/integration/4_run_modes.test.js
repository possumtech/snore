...
test("§4.4 Run Modes — continue, new, lite, fork", async (t) => {
	...
	t.before(async () => {
		...
		await tdb.db.upsert_project.run({ id: "p1", path: "/tmp/rm-test", name: "RM" });
	...
	await t.test("continue: same run, same alias", async () => {
		...
		const continued = await loop.run("ask", "s1", null, "continue", null, first.run, {});
	...
	await t.test("new: fresh run, new alias", async () => {
		...
		assert.notStrictEqual(second.run, first.run, "new run gets different alias");
	});
	...
	await t.test("fork: new run with parent pointer", async () => {
		...
		const originalRow = await tdb.db.get_run_by_alias.get({ alias: original.run });
		assert.strictEqual(forkedRow.parent_run_id, originalRow.id, "fork points to parent");
	});

	await t.test("fork: non-existent run throws", async () => {
		await assert.rejects(
			loop.run("ask", "s1", null, "fork", null, "nonexistent_99", { fork: true }),
			/not found/,
		);
	});
});
