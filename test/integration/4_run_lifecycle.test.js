...
test("§4.3 Run Lifecycle — status transitions", async (t) => {
	...
	t.before(async () => {
		...
		await tdb.db.upsert_project.run({ id: "p1", path: "/tmp/lc-test", name: "LC" });
	...
	await t.test("abort transitions to aborted", async () => {
		...
		await tdb.db.update_run_status.run({ id: (await tdb.db.get_run_by_alias.get({ alias: result.run })).id, status: "aborted" });

		const runRow = await tdb.db.get_run_by_alias.get({ alias: result.run });
		assert.strictEqual(runRow.status, "aborted");
	});

	await t.test("findings gate blocks continuation when findings exist", async () => {
		...
		const first = await loop.run("act", "s1", null, "make edit", null, null, {});
		...
		const turnRow = await tdb.db.create_empty_turn.get({ run_id: runRow.id, sequence: 99 });
		...
		const second = await loop.run("act", "s1", null, "continue", null, first.run, {});
