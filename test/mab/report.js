/**
 * Report generator for MemoryAgentBench results.
 *
 * Reads results from a NDJSON results file and prints
 * a formatted summary to stdout.
 */
import fs from "node:fs/promises";

/**
 * @param {{ split: string, rowIndex: number, source: string, score: object, usage: object, timing: object, questions: object[] }[]} results
 */
export function printReport(results) {
	...
	for (const r of results) {
		splits[r.split] ??= { passed: 0, total: 0, rows: 0 };
		splits[r.split].passed += r.score.passed;
		splits[r.split].total += r.score.total;
		splits[r.split].rows++;
	...
	console.log("\n╔══════════════════════════════════════════════════════════╗");
	console.log("║            MemoryAgentBench — Results Report            ║");
	console.log("╚══════════════════════════════════════════════════════════╝\n");

	console.log("Per-Split Accuracy:");
	console.log("─".repeat(58));
	for (const [name, s] of Object.entries(splits).toSorted((a, b) =>
	...
	// Per-row detail for failures
	const failures = results.flatMap((r) =>
		r.questions
			.filter((q) => !q.pass)
			.map((q) => ({ split: r.split, row: r.rowIndex, ...q })),
	);
	...
	if (failures.length > 0 && failures.length <= 50) {
		...
		for (const f of failures.slice(0, 30)) {
			console.log(`  [${f.split}:${f.row}] Q: ${f.question.slice(0, 70)}`);
