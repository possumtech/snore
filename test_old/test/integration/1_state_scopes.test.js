...
test("§1 State Scopes — promotion isolation", async (t) => {
	...
	await t.test(
		"client promotions are project-scoped, visible across runs",
		async () => {
	...
	await t.test("editor promotions have no run_id", async () => {
		const _file = await tdb.db.get_repo_map_file.get({
			project_id: "p1",
			path: "src/a.js",
			run_id: "r1",
		});
		...
		assert.strictEqual(
			after.has_editor_promotion,
			1,
			"editor promotion present",
		);
		...
		assert.strictEqual(
			cleared.has_editor_promotion,
			0,
			"editor promotion cleared",
		);
	});
});
