...
Allowed: `<unknown/>` `<read/>` `<env/>` `<ask_user/>` `<search/>` `<write/>` `<move/>` `<copy/>` `<drop/>` `<delete/>` `<run/>` `<update/>` `<summary/>`
...
## <drop path="[path/to/file]"/> - Forget a file or entry
Example: <drop path="src/config.js"/>
Example: <drop path="unknown://42"/>
* <drop/> removes the file or entry from context, but does not delete it
* A dropped file or entry can be restored with <read/>
...
Example: <delete path="known://api_*" value="v1" keys/> (list all api entries with v1 in known that would be deleted)
